...
export class AuthenticationService {
  ...
  public currentLogedInUser = this.currentLoginUserSubject.asObservable();
  ...
  populateLogedInUserInfo() {
    if (this.jwtStorageService.getToken()) {
      ...
      this.currentLoginUserSubject.next(decodedToken);
  ...
  private decodeJWT(): User|null {
    ...
    const token = this.jwtStorageService.getToken();
    // check token is not null and toke is not expired
